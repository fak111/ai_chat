...
A宝 是一个 AI 群聊移动应用，让熟人网络可以在群聊中与 AI 互动。已完成 MVP 开发并部署上线。


## 最重要的事情
1. **TDD 先行** - fix/feat 必须先写失败测试，设计规范、覆盖率高的的测试遵循红黄绿，再实现代码。
2. **原子提交** - 每个 commit 只做一件事，可独立回滚
3. **文档驱动** - 所有feat改动关联 doc/下面的文档，需要仔细调研代码库，多让AI大哥输出表格、流程图、ASCII原型图，方便沟通理解。
4. **知识沉淀** - 任何有价值的对话迭代都要沉淀到 CLAUDE.md（比如我们成功解决了个问题，拿捏不准主动问我）
5. **利用先用的工具** - 不重复造轮子，调研现有的方案实现方式，会开车>会修车。
6. **任务彻底结束后，追加一句** - 主人，用不用我沉淀 or git提交？**
...
| 后端 | Spring Boot 3.x + WebSocket | Docker 容器化 |
| 数据库 | PostgreSQL 15 (自建) | Docker 容器，原 Supabase 已弃用 |
| 缓存 | Caffeine (本地优先) + Redis 7 (自建) | Docker 容器，原 Upstash 已弃用 |
| AI 服务 | DeepSeek API | 外部依赖，需 API Key |
| 邮箱服务 | Resend | 外部依赖，需 API Key（可手动验证绕过） |
| 对象存储 | Cloudflare R2 | 规划中，尚未实现 |
| 部署 | Docker + 自有服务器 | 原 Railway 已弃用，改为自建部署 |
...
- **版本号**: 采用 `vX.Y` 格式，X 为大版本（结构性变更），Y 为小版本（内容修订）
- **创建时间 / 最后更新**: `YYYY-MM-DD` 格式
- 每次修改文档时**必须**同步更新`版本`和`最后更新`字段
...
已完成:
- Phase 0: 设计阶段
- Phase 1 (MVP): 用户系统 + 群聊系统 + AI 交互 + 自建服务器部署

计划阶段:
- Phase 2: 历史消息分页、邀请码分享优化、对象存储
...
├── Docker: abao-server    (Spring Boot, 端口 8080, 仅 127.0.0.1)
...
```bash
# 一键部署（本地构建 → 上传镜像 → 服务器重启）
./deploy.sh
```

流程: 本地 `docker build` (linux/amd64) → `docker save` 导出 → SCP 上传 → 服务器 `docker load` → `docker compose up -d`
